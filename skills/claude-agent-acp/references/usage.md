...
- The adapter passes inherited environment variables to the Claude Agent SDK and Claude Code runtime.
...
If building a custom smoke client, keep logs on stderr and reserve stdout/stdin for the ACP transport. The adapter redirects normal console output away from stdout so ACP framing is not corrupted.
