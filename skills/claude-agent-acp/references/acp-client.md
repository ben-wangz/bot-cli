...
```bash
node /absolute/path/to/skills/claude-agent-acp/scripts/prompt-claude-agent-acp.mjs \
...
If the ACP SDK cannot be resolved automatically, pass the installed `acp.js` path with `--sdk-path` after inspecting the global package location.
