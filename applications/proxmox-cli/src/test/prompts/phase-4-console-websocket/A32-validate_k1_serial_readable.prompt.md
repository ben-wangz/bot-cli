...
```text
You are a test execution agent. Run the A32 `validate_k1_serial_readable` positive-path test.
...
Command:
go run ./cmd/proxmox-cli --api-base "${PVE_API_BASE_URL%/}/api2/json" --insecure-tls --output json action validate_k1_serial_readable --node "$TEST_NODE" --vmid "$TEST_VMID" --script "<ENTER>" --timeout-seconds 30

Success criteria:
- exit code = 0
- JSON field `action == "validate_k1_serial_readable"`
- JSON field `ok == true`
- JSON field `result.readable == true`
- JSON contains `result.transcript_tail`
...
- Never depend on shell login state from another prompt run.
