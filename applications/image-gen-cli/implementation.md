...
- `workflow` 子命令：承载最小端到端流程（streaming 首图 + non-streaming 续链第二张图）。
...
参考现有应用 `tests/prompts` 方式，v0 仅保留最小链路用例：

- 用例 A：streaming 生成第一张图并落盘。
- 用例 B：non-streaming + `previous_response_id` 续链生成第二张图并落盘。
...
- 与 `proxmox-cli` 对齐：
  - internal 分层清晰（cli/capability/output/workflow/apperr）
  - workflow 用于组合链路回归
